...
const getOptions = ({ test, dev, build, cache, include, exclude, stylelintPath, formatter, lintInWorker, lintOnStart, lintDirtyOnly, emitError, emitErrorAsWarning, emitWarning, emitWarningAsError, ...stylelintOptions }) => ({
	test: test ?? false,
	dev: dev ?? true,
	build: build ?? false,
	cache: cache ?? true,
...
const getFilter = (options) => (0, _rollup_pluginutils.createFilter)(options.include, options.exclude);
const initializeStylelint = async (options) => {
	...
	try {
		...
		return {
			stylelintInstance,
			formatter: typeof formatter === "string" ? await stylelintInstance.formatters[formatter] : formatter
		};
	} catch (error) {
...
const getFilePath = (id) => (0, _rollup_pluginutils.normalizePath)(id).split("?")[0];
...
const lintFiles = async ({ files, stylelintInstance, formatter, options }, context) => await stylelintInstance.lint({
	...
	return log(formatter(results, linterResult), (() => {
		if (linterResult.errored) return options.emitErrorAsWarning ? "warning" : "error";
...
Object.defineProperty(exports, "initializeStylelint", {
	enumerable: true,
	get: function() {
		return initializeStylelint;
	}
});
