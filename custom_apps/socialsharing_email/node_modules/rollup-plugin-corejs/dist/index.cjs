...
var compat = require('core-js-compat');
...
(function (PolyfillType) {
    PolyfillType[PolyfillType["CallWithArguments"] = 0] = "CallWithArguments";
    PolyfillType[PolyfillType["StaticMember"] = 1] = "StaticMember";
    PolyfillType[PolyfillType["GenericMethod"] = 2] = "GenericMethod";
    PolyfillType[PolyfillType["GenericProperty"] = 3] = "GenericProperty";
    PolyfillType[PolyfillType["Global"] = 4] = "Global";
...
(() => [
    ...
    detectableModules.push(["es.typed-array.from", PT.StaticMember, name, "from"], ["es.typed-array.of", PT.StaticMember, name, "of"], [module, PT.Global, name]);
}))();
(() => [
    "es.math.acosh",
    "es.math.asinh",
...
class Walker {
    ...
    _properties = new Map();
    _calls = new Map();
    registerModules(modules, filterModules) {
        const usedModules = [];
        ...
        return {
            ...
            MemberExpression(path, state) {
                if (path.node.object.type === "Identifier") {
                    const property = path.node.property.type === "Identifier"
...
function filterModules(modules, ast) {
    const realModules = [];
    modules.forEach((m) => {
        if (m in aliasModules)
            realModules.push(...aliasModules[m]);
        else
            realModules.push(m);
    });
    const walker = new Walker();
    walker.registerModules(detectableModules, realModules);
    estreeToolkit.traverse(ast, Object.assign(walker.getVisitors(), {
        $: {
            scope: true,
        },
    }), walker);
 ...
 * SPDX-License-Identifier: EUPL-1.2
 */
function corejsPlugin(options = { modules: "core-js/es" }) {
    const config = {
...
exports.corejsPlugin = corejsPlugin;
