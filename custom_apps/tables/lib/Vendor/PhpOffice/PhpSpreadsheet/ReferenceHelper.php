...
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\AutoFilter;
...
class ReferenceHelper
{
    ...
    protected function adjustPageBreaks(Worksheet $worksheet, int $numberOfColumns, int $numberOfRows) : void
    {
        ...
        foreach ($aBreaks as $cellAddress => $value) {
            ...
            if ($cellReferenceHelper->cellAddressInDeleteRange($cellAddress) === \true) {
            ...
            foreach ($cfRules as &$cfRule) {
                ...
                foreach ($conditions as &$condition) {
                    if (\is_string($condition)) {
                        /** @var CellReferenceHelper */
                        $cellReferenceHelper = $this->cellReferenceHelper;
                        $condition = $this->updateFormulaReferences($condition, $cellReferenceHelper->beforeCellAddress(), $numberOfColumns, $numberOfRows, $worksheet->getTitle(), \true);
                    }
                }
    ...
    protected function adjustDataValidations(Worksheet $worksheet, int $numberOfColumns, int $numberOfRows, string $beforeCellAddress) : void
    {
        ...
        foreach ($aDataValidationCollection as $cellAddress => $dataValidation) {
            ...
            if ($formula !== '') {
                $dataValidation->setFormula1($this->updateFormulaReferences($formula, $beforeCellAddress, $numberOfColumns, $numberOfRows, $worksheet->getTitle(), \true));
            }
            $formula = $dataValidation->getFormula2();
            if ($formula !== '') {
                $dataValidation->setFormula2($this->updateFormulaReferences($formula, $beforeCellAddress, $numberOfColumns, $numberOfRows, $worksheet->getTitle(), \true));
            }
        ...
        if ($numberOfRows >= 0 && $numberOfColumns >= 0) {
            foreach ($aProtectedCells as $key2 => $value) {
                ...
                if ($key2 !== $newKey) {
                    ...
                    $worksheet->protectCells($newKey, $value->getPassword(), \true, $value->getName(), $value->getSecurityDescriptor());
                }
            }
        } else {
            foreach ($aProtectedCells as $key2 => $value) {
        ...
        while ($coordinate = \array_pop($allCoordinates)) {
            ...
            if ($cellIndex >= $beforeColumn && $cell->getRow() >= $beforeRow) {
                ...
                if ($cell->getDataType() === DataType::TYPE_FORMULA) {
                    ...
                    $worksheet->getCell($newCoordinate)->setValue($this->updateFormulaReferences($cell->getValueString(), $beforeCellAddress, $numberOfColumns, $numberOfRows, $worksheet->getTitle(), \true));
                } else {
                ...
                if ($cell->getDataType() === DataType::TYPE_FORMULA) {
                    // Formula should be adjusted
                    $cell->setValue($this->updateFormulaReferences($cell->getValueString(), $beforeCellAddress, $numberOfColumns, $numberOfRows, $worksheet->getTitle(), \true));
                }
            }
        }
    ...
    {
        return $match === null || $match === '' || $match === "'￼'" || $match === "'￻'" || \strcasecmp(\trim($match, "'"), $worksheetName) === 0;
    }
    private static function sheetnameBeforeCells(string $match, string $worksheetName, string $cells) : string
    {
        $toString = $match > '' ? "{$match}!" : '';
        return \str_replace(["￼", "'￻'"], $worksheetName, $toString) . $cells;
    }
    /**
    ...
    {
        $callback = fn(array $matches): string => \strcasecmp(\trim($matches[2], "'"), $worksheetName) === 0 ? $matches[2][0] === "'" ? "'￼'!" : "'￻'!" : "'�'!";
        if ($this->cellReferenceHelper === null || $this->cellReferenceHelper->refreshRequired($beforeCellAddress, $numberOfColumns, $numberOfRows)) {
            $this->cellReferenceHelper = new CellReferenceHelper($beforeCellAddress, $numberOfColumns, $numberOfRows);
        }
            ...
            if ($i) {
                ...
                if ($matchCount > 0) {
                    foreach ($matches as $match) {
                        ...
                        $modified3 = \substr($this->updateCellReference('$A' . $match[3], $includeAbsoluteReferences, $onlyAbsoluteReferences, \true), 2);
                ...
                if ($matchCount > 0) {
                    foreach ($matches as $match) {
                        $fromString = self::sheetnameBeforeCells($match[2], $worksheetName, "{$match[3]}:{$match[4]}");
                        $modified3 = \substr($this->updateCellReference($match[3] . '$1', $includeAbsoluteReferences, $onlyAbsoluteReferences, \true), 0, -2);
                ...
                if ($matchCount > 0) {
                    foreach ($matches as $match) {
                        $fromString = self::sheetnameBeforeCells($match[2], $worksheetName, "{$match[3]}:{$match[4]}");
                        $modified3 = $this->updateCellReference($match[3], $includeAbsoluteReferences, $onlyAbsoluteReferences, \true);
    ...
    private function updateNamedRange(DefinedName $definedName, Worksheet $worksheet, string $beforeCellAddress, int $numberOfColumns, int $numberOfRows) : void
    {
        ...
        if ($definedName->getWorksheet() === $worksheet) {
             ...
             */
            if ($asFormula === \true) {
                $formula = $this->updateFormulaReferences($cellAddress, $beforeCellAddress, $numberOfColumns, $numberOfRows, $worksheet->getTitle(), \true, \true);
        ...
        for ($i = 0; $i < $ic; ++$i) {
            ...
            for ($j = 0; $j < $jc; ++$j) {
                ...
                if (\ctype_alpha($range[$i][$j])) {
                    $range[$i][$j] = Coordinate::coordinateFromString($cellReferenceHelper->updateCellReference($range[$i][$j] . '1', $includeAbsoluteReferences, $onlyAbsoluteReferences, null))[0];
                } elseif (\ctype_digit($range[$i][$j])) {
                    $range[$i][$j] = Coordinate::coordinateFromString($cellReferenceHelper->updateCellReference('A' . $range[$i][$j], $includeAbsoluteReferences, $onlyAbsoluteReferences, null))[1];
                } else {
