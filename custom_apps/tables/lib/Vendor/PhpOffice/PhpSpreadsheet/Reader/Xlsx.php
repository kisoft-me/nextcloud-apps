...
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Cell\Hyperlink;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Comment;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\DefinedName;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Reader\Security\XmlScanner;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Reader\Xlsx\AutoFilter;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Reader\Xlsx\Chart;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Reader\Xlsx\ColumnAndRowAttributes;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Reader\Xlsx\ConditionalStyles;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Reader\Xlsx\DataValidations;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Reader\Xlsx\Hyperlinks;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Reader\Xlsx\Namespaces;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Reader\Xlsx\PageSetup;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Reader\Xlsx\Properties as PropertyReader;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Reader\Xlsx\SharedFormula;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Reader\Xlsx\SheetViewOptions;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Reader\Xlsx\SheetViews;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Reader\Xlsx\Styles;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Reader\Xlsx\TableReader;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Reader\Xlsx\Theme;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Reader\Xlsx\WorkbookView;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\ReferenceHelper;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\RichText\RichText;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Shared\Date;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Shared\Drawing;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Shared\File;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Shared\Font;
...
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Style\Color;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Style\Font as StyleFont;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Style\NumberFormat;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Style\Style;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\HeaderFooterDrawing;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\Table\TableDxfsStyle;
...
class Xlsx extends BaseReader
{
    ...
    private function castToFormula(?SimpleXMLElement $c, string $r, string &$cellDataType, mixed &$value, mixed &$calculatedValue, string $castBaseType, bool $updateSharedCells = \true) : void
    {
        if ($c === null) {
            return;
        }
                    ...
                    if ($xmlWorkbookNS->sheets) {
                        foreach ($xmlWorkbookNS->sheets->sheet as $eleSheet) {
                            ...
                            if ($xmlSheetNS) {
                                ...
                                (new ColumnAndRowAttributes($docSheet, $xmlSheetNS))->load($this->getReadFilter(), $this->readDataOnly, $this->ignoreRowsWithNoCells);
                            }
                                    ...
                                    foreach ($row->c as $c) {
                                        ...
                                        if ($this->readEmptyCells || $value !== null && $value !== '') {
                                            ...
                                            if (!$this->readDataOnly) {
                                                ...
                                                // issue 3495
                                                if ($cellDataType === DataType::TYPE_FORMULA && $styles[$cAttrS]->quotePrefix === \true) {
                                                    //* @phpstan-ignore-line
                                        ...
                                        if (isset($shape['style'])) {
                                            ...
                                            if ($column !== null && $row !== null) {
                                                ...
                                                if (isset($fillImageRelId, $drowingImages[$fillImageRelId])) {
                                                    $objDrawing = new \OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\Drawing();
                                        ...
                                        if ($xmlDrawingChildren->oneCellAnchor) {
                                            foreach ($xmlDrawingChildren->oneCellAnchor as $oneCellAnchor) {
                                                ...
                                                if ($oneCellAnchor->pic->blipFill) {
                                                    ...
                                                    $embedImageKey = self::getArrayItemString(self::getAttributes($blip, $xmlNamespaceBase), 'embed');
                                                    if (isset($images[$embedImageKey])) {
                                                        $objDrawing->setPath('zip://' . File::realpath($filename) . '#' . $images[$embedImageKey], \false, $zip);
                                                    } else {
                                                        $linkImageKey = self::getArrayItemString($blip->attributes('http://schemas.openxmlformats.org/officeDocument/2006/relationships'), 'link');
                                ...
                                if ((string) $definedName['localSheetId'] != '') {
                                    ...
                                    switch ((string) $definedName['name']) {
                                        ...
                                        default:
                                            if ($mapSheetId[(int) $definedName['localSheetId']] !== null) {
                                                $range = Worksheet::extractSheetTitle($extractedRange, \true);
                                    ...
                                    if (\str_contains((string) $definedName, '!')) {
                                        ...
                                        if (\is_array($definedNameValueParts)) {
                                            // Extract sheet name
                                            [$extractedSheetName] = Worksheet::extractSheetTitle((string) $definedNameValueParts[0], \true, \true);
                                            // Locate sheet
    ...
    private function readHyperLinkDrawing(\OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\Drawing $objDrawing, SimpleXMLElement $cellAnchor, array $hyperlinks) : void
    {
        ...
        $hyperlink = new Hyperlink($hyperlinks[$hlinkId], self::getArrayItemString(self::getAttributes($cellAnchor->pic->nvPicPr->cNvPr), 'name'));
    ...
    private function readProtection(Spreadsheet $excel, SimpleXMLElement $xmlWorkbook) : void
    {
        ...
        $security->setLockRevision(self::getLockValue($xmlWorkbook->workbookProtection, 'lockRevision'));
        $security->setLockStructure(self::getLockValue($xmlWorkbook->workbookProtection, 'lockStructure'));
        $security->setLockWindows(self::getLockValue($xmlWorkbook->workbookProtection, 'lockWindows'));
        if ($xmlWorkbook->workbookProtection['revisionsPassword']) {
            $security->setRevisionsPassword((string) $xmlWorkbook->workbookProtection['revisionsPassword'], \true);
        }
        if ($xmlWorkbook->workbookProtection['revisionsAlgorithmName']) {
            $security->setRevisionsAlgorithmName((string) $xmlWorkbook->workbookProtection['revisionsAlgorithmName']);
        }
        if ($xmlWorkbook->workbookProtection['revisionsSaltValue']) {
            $security->setRevisionsSaltValue((string) $xmlWorkbook->workbookProtection['revisionsSaltValue'], \false);
        }
        if ($xmlWorkbook->workbookProtection['revisionsSpinCount']) {
            $security->setRevisionsSpinCount((int) $xmlWorkbook->workbookProtection['revisionsSpinCount']);
        }
        if ($xmlWorkbook->workbookProtection['revisionsHashValue']) {
            if ($security->advancedRevisionsPassword()) {
                $security->setRevisionsPassword((string) $xmlWorkbook->workbookProtection['revisionsHashValue'], \true);
            }
        }
        if ($xmlWorkbook->workbookProtection['workbookPassword']) {
            $security->setWorkbookPassword((string) $xmlWorkbook->workbookProtection['workbookPassword'], \true);
        }
        ...
        if ($xmlWorkbook->workbookProtection['workbookSaltValue']) {
            $security->setWorkbookSaltValue((string) $xmlWorkbook->workbookProtection['workbookSaltValue'], \false);
        }
        if ($xmlWorkbook->workbookProtection['workbookSpinCount']) {
            $security->setWorkbookSpinCount((int) $xmlWorkbook->workbookProtection['workbookSpinCount']);
        }
        if ($xmlWorkbook->workbookProtection['workbookHashValue']) {
            if ($security->advancedPassword()) {
                $security->setWorkbookPassword((string) $xmlWorkbook->workbookProtection['workbookHashValue'], \true);
            }
        }
    }
    ...
    private function readSheetProtection(Worksheet $docSheet, SimpleXMLElement $xmlSheet) : void
    {
        ...
        if ($xmlSheet->protectedRanges->protectedRange) {
            foreach ($xmlSheet->protectedRanges->protectedRange as $protectedRange) {
                $docSheet->protectCells((string) $protectedRange['sqref'], (string) $protectedRange['password'], \true, (string) $protectedRange['name'], (string) $protectedRange['securityDescriptor']);
            }
        }
    }
                ...
                if (\preg_match($pattern1, $sqref1, $matches) === 1) {
                    ...
                    for ($row = $firstRow; $row <= $lastRow; ++$row) {
                        for ($col = $firstCol; $col !== $lastCol; StringHelper::stringIncrement($col)) {
                            ...
                            if ($formulaRange === '1') {
                                $sheet->getCell("{$col}{$row}")->getIgnoredErrors()->setFormulaRange(\true);
                            }
