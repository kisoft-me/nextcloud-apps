...
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Shared\Escher as SharedEscher;
...
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Shared\Xls as SharedXls;
        ...
        foreach ($xls->sheets as $sheet) {
            ...
            if (!empty($xls->cellNotes)) {
                foreach ($xls->cellNotes as $note => $noteDetails) {
                    ...
                    $xls->phpSheet->getComment($cellAddress)->setAuthor($noteDetails['author'])->setText($xls->parseRichText($tempDetails));
                }
            }
            if ($selectedCells !== '') {
        ...
        foreach ($xls->definedname as $definedName) {
            ...
            if ($definedName['isBuiltInName']) {
                switch ($definedName['name']) {
                    case \pack('C', 0x6):
                        ...
                        foreach ($ranges as $range) {
                            ...
                            //        Foo!$C$7:$J$66
                            //        Bar!$A$1:$IV$2
                            $explodes = Worksheet::extractSheetTitle($range, \true, \true);
                ...
                if (\str_contains($formula, '!')) {
                    $explodes = Worksheet::extractSheetTitle($formula, \true, \true);
                    $docSheet = $xls->spreadsheet->getSheetByName($explodes[0]);
                    if ($docSheet) {
                        $extractedRange = $explodes[1];
