...
class ConditionalFormatting extends Xls
{
    ...
    protected function readCFHeader2(Xls $xls) : array
    {
        ...
        $cellRangeAddressList = $xls->version == self::XLS_BIFF8 ? Biff8::readBIFF8CellRangeAddressList(\substr($recordData, 12)) : Biff5::readBIFF5CellRangeAddressList(\substr($recordData, 12));
    ...
    protected function readCFRule2(array $cellRangeAddresses, Xls $xls) : void
    {
        ...
        $hasBorderLeft = !(bool) (0x400 & $options);
        $hasBorderRight = !(bool) (0x800 & $options);
        $hasBorderTop = !(bool) (0x1000 & $options);
        $hasBorderBottom = !(bool) (0x2000 & $options);
        $offset = 12;
    ...
    private function getCFFontStyle(string $options, Style $style, Xls $xls) : void
    {
        ...
        if (($options88 & 2) === 0) {
            ...
            if (($options68 & 2) !== 0) {
                $style->getFont()->setItalic(\true);
            }
        }
        if (($options88 & 0x80) === 0) {
            if (($options68 & 0x80) !== 0) {
                $style->getFont()->setStrikethrough(\true);
            }
        }
    ...
    private function getCFBorderStyle(string $options, Style $style, bool $hasBorderLeft, bool $hasBorderRight, bool $hasBorderTop, bool $hasBorderBottom, Xls $xls) : void
    {
        ...
        if ($hasBorderLeft) {
            $style->getBorders()->getLeft()->setBorderStyle(self::BORDER_STYLE_MAP[$left]);
            $style->getBorders()->getLeft()->getColor()->setRGB(Color::map($leftc, $xls->palette, $xls->version)['rgb']);
        }
        if ($hasBorderRight) {
            $style->getBorders()->getRight()->setBorderStyle(self::BORDER_STYLE_MAP[$right]);
            $style->getBorders()->getRight()->getColor()->setRGB(Color::map($rightc, $xls->palette, $xls->version)['rgb']);
        }
        if ($hasBorderTop) {
            $style->getBorders()->getTop()->setBorderStyle(self::BORDER_STYLE_MAP[$top]);
            $style->getBorders()->getTop()->getColor()->setRGB(Color::map($topc, $xls->palette, $xls->version)['rgb']);
        }
        if ($hasBorderBottom) {
