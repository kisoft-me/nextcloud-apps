...
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Writer\Xls\Worksheet as XlsWorksheet;
/** @internal */
class DataValidationHelper extends Xls
{
     ...
     */
    private static array $types = [0x0 => DataValidation::TYPE_NONE, 0x1 => DataValidation::TYPE_WHOLE, 0x2 => DataValidation::TYPE_DECIMAL, 0x3 => DataValidation::TYPE_LIST, 0x4 => DataValidation::TYPE_DATE, 0x5 => DataValidation::TYPE_TIME, 0x6 => DataValidation::TYPE_TEXTLENGTH, 0x7 => DataValidation::TYPE_CUSTOM];
    /**
     * @var array<int, string>
     */
    private static array $errorStyles = [0x0 => DataValidation::STYLE_STOP, 0x1 => DataValidation::STYLE_WARNING, 0x2 => DataValidation::STYLE_INFORMATION];
    /**
     * @var array<int, string>
     */
    private static array $operators = [0x0 => DataValidation::OPERATOR_BETWEEN, 0x1 => DataValidation::OPERATOR_NOTBETWEEN, 0x2 => DataValidation::OPERATOR_EQUAL, 0x3 => DataValidation::OPERATOR_NOTEQUAL, 0x4 => DataValidation::OPERATOR_GREATERTHAN, 0x5 => DataValidation::OPERATOR_LESSTHAN, 0x6 => DataValidation::OPERATOR_GREATERTHANOREQUAL, 0x7 => DataValidation::OPERATOR_LESSTHANOREQUAL];
    ...
    protected function readDataValidation2(Xls $xls) : void
    {
        ...
        foreach ($cellRangeAddresses as $cellRange) {
            $cellRange = \preg_replace(["/([a-z]+)1:([a-z]+){$maxXlsRow}/i", "/([a-z]+\\d+):([a-z]+){$maxXlsRow}/i", "/A(\\d+):{$maxXlsColumnString}(\\d+)/i", "/([a-z]+\\d+):{$maxXlsColumnString}(\\d+)/i"], ['$1:$2', '$1:${2}' . $maxRow, '$1:$2', '$1:' . $maxCol . '$2'], $cellRange) ?? $cellRange;
