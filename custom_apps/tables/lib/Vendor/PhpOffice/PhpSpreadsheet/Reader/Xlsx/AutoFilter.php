...
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\AutoFilter\Column;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\AutoFilter\Column\Rule;
    ...
    private function readCustomAutoFilter(?SimpleXMLElement $filterColumn, Column $column) : void
    {
        if (isset($filterColumn, $filterColumn->customFilters)) {
            ...
            foreach ($customFilters->customFilter as $filterRule) {
                ...
                $column->createRule()->setRule((string) $attr2['operator'], (string) $attr2['val'])->setRuleType(Rule::AUTOFILTER_RULETYPE_CUSTOMFILTER);
            }
        }
    }
    private function readDynamicAutoFilter(?SimpleXMLElement $filterColumn, Column $column) : void
    {
        if (isset($filterColumn, $filterColumn->dynamicFilter)) {
            ...
            foreach ($filterColumn->dynamicFilter as $filterRule) {
                ...
                $column->createRule()->setRule('', (string) ($attr2['val'] ?? ''), (string) ($attr2['type'] ?? ''))->setRuleType(Rule::AUTOFILTER_RULETYPE_DYNAMICFILTER);
                if (isset($attr2['val'])) {
    ...
    private function readTopTenAutoFilter(?SimpleXMLElement $filterColumn, Column $column) : void
    {
        if (isset($filterColumn, $filterColumn->top10)) {
            ...
            foreach ($filterColumn->top10 as $filterRule) {
                $attr2 = $filterRule->attributes() ?? [];
                $column->createRule()->setRule(isset($attr2['percent']) && (string) $attr2['percent'] === '1' ? Rule::AUTOFILTER_COLUMN_RULE_TOPTEN_PERCENT : Rule::AUTOFILTER_COLUMN_RULE_TOPTEN_BY_VALUE, (string) ($attr2['val'] ?? ''), isset($attr2['top']) && (string) $attr2['top'] === '1' ? Rule::AUTOFILTER_COLUMN_RULE_TOPTEN_TOP : Rule::AUTOFILTER_COLUMN_RULE_TOPTEN_BOTTOM)->setRuleType(Rule::AUTOFILTER_RULETYPE_TOPTENFILTER);
            }
        }
    }
}
