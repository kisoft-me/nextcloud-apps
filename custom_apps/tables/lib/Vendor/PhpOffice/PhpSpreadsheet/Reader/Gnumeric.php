...
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Reader\Gnumeric\PageSetup;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Reader\Gnumeric\Properties;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Reader\Gnumeric\Styles;
...
class Gnumeric extends BaseReader
{
    ...
    public function listWorksheetInfo(string $filename) : array
    {
        ...
        while ($xml->read()) {
            if (self::matchXml($xml, 'Sheet')) {
                $tmpInfo = ['worksheetName' => '', 'lastColumnLetter' => 'A', 'lastColumnIndex' => 0, 'totalRows' => 0, 'totalColumns' => 0, 'sheetState' => Worksheet::SHEETSTATE_VISIBLE];
    ...
    {
        $this->spreadsheet->getActiveSheet()->getColumnDimension(Coordinate::stringFromColumnIndex($whichColumn + 1))->setWidth($defaultWidth);
    }
    private function setColumnInvisible(int $whichColumn) : void
    {
        $this->spreadsheet->getActiveSheet()->getColumnDimension(Coordinate::stringFromColumnIndex($whichColumn + 1))->setVisible(\false);
    }
    ...
    {
        $this->spreadsheet->getActiveSheet()->getRowDimension($whichRow)->setVisible(\false);
    }
    ...
    private function processDefinedNames(?SimpleXMLElement $gnmXML) : void
    {
        ...
        if ($gnmXML !== null && isset($gnmXML->Names)) {
            foreach ($gnmXML->Names->Name as $definedName) {
                ...
                [$worksheetName] = Worksheet::extractSheetTitle($value, \true, \true);
    ...
    private function loadCell(SimpleXMLElement $cell, string $worksheetName, SimpleXMLElement $cellAttributes, string $column, int $row) : void
    {
        ...
        if ($ExprID > '') {
            if ((string) $cell > '') {
                // Formula
                $this->expressions[$ExprID] = ['column' => (int) $cellAttributes->Col, 'row' => (int) $cellAttributes->Row, 'formula' => (string) $cell];
            } else {
                // Shared Formula
    ...
    private function getArrayFormulaRange(string $column, int $row, int $cols, int $rows) : string
    {
        ...
        $arrayFormulaRange .= ':' . Coordinate::stringFromColumnIndex(Coordinate::columnIndexFromString($column) + $cols - 1) . (string) ($row + $rows - 1);
