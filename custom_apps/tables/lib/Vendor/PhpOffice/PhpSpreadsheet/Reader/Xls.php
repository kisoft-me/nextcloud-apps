...
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Reader\Xls\Style\CellFont;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Reader\Xls\Style\FillPattern;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\RichText\RichText;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Shared\CodePage;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Shared\Date;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Shared\Escher;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Shared\File;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Shared\OLE;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Shared\OLERead;
...
class Xls extends XlsBase
{
    ...
    protected function readTextObject() : void
    {
        ...
        $this->textObjects[$this->textObjRef] = ['text' => $textStr, 'format' => \substr($textRecordData, $tempSplice[1], $cbRuns), 'alignment' => $grbitOpts, 'rotation' => $rot];
    }
    /**
     * Read BOF.
     */
    ...
    protected function readXf() : void
    {
        ...
        if (!$this->readDataOnly) {
            ...
            if ($this->version == self::XLS_BIFF8) {
                ...
                $diagonalUp = (self::HIGH_ORDER_BIT & self::getInt4d($recordData, 10)) >> 31 ? \true : \false;
                if ($diagonalUp === \false) {
                ...
                $objStyle->getFill()->setFillType(FillPattern::lookup((0x3f0000 & $borderAndBackground) >> 16));
    ...
    protected function readFooter() : void
    {
        ...
        if (!$this->readDataOnly) {
            ...
            if ($recordData) {
                ...
                /** @var string */
                $temp = $string['value'];
                $this->phpSheet->getHeaderFooter()->setOddFooter($temp);
                $this->phpSheet->getHeaderFooter()->setEvenFooter($temp);
            }
        }
    }
    /**
    ...
    protected function readRow() : void
    {
        ...
        if (!$this->readDataOnly) {
            ...
            if (!$useDefaultHeight) {
                if ($this->phpSheet->getDefaultRowDimension()->getRowHeight() > 0) {
                    $this->phpSheet->getRowDimension($r + 1)->setCustomFormat(\true, $height === 255 ? -1 : $height / 20);
                } else {
    ...
    protected function readMergedCells() : void
    {
        ...
        if ($this->version == self::XLS_BIFF8 && !$this->readDataOnly) {
            ...
            foreach ($cellRangeAddressList['cellRangeAddresses'] as $cellRangeAddress) {
                ...
                if (\str_contains($cellRangeAddress, ':') && $this->includeCellRangeFiltered($cellRangeAddress)) {
    ...
    protected function readRangeProtection() : void
    {
        ...
        if (!$this->readDataOnly) {
            ...
            if ($cellRanges) {
                $this->phpSheet->protectCells(\implode(' ', $cellRanges), $wPassword === 0 ? '' : \strtoupper(\dechex($wPassword)), \true);
            }
        }
    }
    /**
