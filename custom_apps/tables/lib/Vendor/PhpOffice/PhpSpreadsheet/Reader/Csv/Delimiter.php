...
namespace OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Reader\Csv;
...
class Delimiter
{
    ...
    public function infer() : ?string
    {
        ...
        foreach (self::POTENTIAL_DELIMITERS as $delimiter) {
            ...
            $meanSquareDeviations[$delimiter] = \array_reduce($series, fn($sum, $value): int|float => $sum + ($value - $median) ** 2) / \count($series);
        }
