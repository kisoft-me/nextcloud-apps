...
use DateTime;
use DateTimeZone;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Cell\AddressHelper;
...
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Reader\Xml\PageSettings;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Reader\Xml\Properties;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Reader\Xml\Style;
...
class Xml extends BaseReader
{
    ...
    {
        $contents = \preg_replace('/&(amp|lt|gt|quot|apos);/', "￾﻿\$1;", \trim($contents)) ?? $contents;
    ...
    private function trySimpleXMLLoadStringPrivate(string $filename, string $fileOrString = 'file') : SimpleXMLElement|bool
    {
        ...
        try {
            ...
            if ($continue) {
                $xml = @\simplexml_load_string($this->getSecurityScannerOrThrow()->scan($data));
            }
        } catch (Throwable $e) {
    ...
    public function loadIntoExisting(string $filename, Spreadsheet $spreadsheet, bool $useContents = \false) : Spreadsheet
    {
        ...
        foreach ($xml_ss->Worksheet as $worksheetx) {
            ...
            $worksheet_ss = self::getAttributes($worksheet, self::NAMESPACES_SS);
            if (isset($this->loadSheetsOnly, $worksheet_ss['Name']) && !\in_array($worksheet_ss['Name'], $this->loadSheetsOnly)) {
                continue;
            }
                                    ...
                                    */
                                    case 'String':
                                        ...
                                        if ($rich) {
                                            ...
                                            $html = new HelperHtml();
                                            $cellValue = $html->toRichTextObject($content, \true);
                                        }
                                        break;
                                    case 'Number':
