...
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Helper\Dimension as HelperDimension;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Reader\Ods\AutoFilter;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Reader\Ods\DefinedNames;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Reader\Ods\FormulaTranslator;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Reader\Ods\PageSettings;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Reader\Ods\Properties as DocumentProperties;
...
class Ods extends BaseReader
{
    ...
    public function listWorksheetInfo(string $filename) : array
    {
        ...
        while ($xml->read()) {
            if ($xml->name === 'style:style') {
                ...
                $tmpInfo = ['worksheetName' => (string) $xml->getAttribute('table:name'), 'lastColumnLetter' => 'A', 'lastColumnIndex' => 0, 'totalRows' => 0, 'totalColumns' => 0, 'sheetState' => $visibility];
            ...
            foreach ($tables as $worksheetDataSet) {
                ...
                foreach ($worksheetDataSet->childNodes as $childNode) {
                    ...
                    switch ($key) {
                        ...
                        case 'table-rows':
                            $this->processTableHeaderRows($childNode, $tableNs, $rowID, $worksheetName, $officeNs, $textNs, $xlinkNs, $spreadsheet);
                            break;
                        case 'table-row-group':
                            $this->processTableRowGroup($childNode, $tableNs, $rowID, $worksheetName, $officeNs, $textNs, $xlinkNs, $spreadsheet);
                            break;
                        case 'table-header-columns':
                        case 'table-columns':
                            $this->processTableHeaderColumns($childNode, $tableNs, $columnWidths, $tableColumnIndex, $spreadsheet);
                            break;
                        case 'table-column-group':
                            $this->processTableColumnGroup($childNode, $tableNs, $columnWidths, $tableColumnIndex, $spreadsheet);
                            break;
                        case 'table-column':
                            $this->processTableColumn($childNode, $tableNs, $columnWidths, $tableColumnIndex, $spreadsheet);
                            break;
                        case 'table-row':
                            $this->processTableRow($childNode, $tableNs, $rowID, $worksheetName, $officeNs, $textNs, $xlinkNs, $spreadsheet);
                            break;
                    }
                }
    ...
    private function processTableHeaderRows(DOMElement $childNode, string $tableNs, int &$rowID, string $worksheetName, string $officeNs, string $textNs, string $xlinkNs, Spreadsheet $spreadsheet) : void
    {
        foreach ($childNode->childNodes as $grandchildNode) {
            ...
            switch ($grandkey) {
                case 'table-row':
                    $this->processTableRow($grandchildNode, $tableNs, $rowID, $worksheetName, $officeNs, $textNs, $xlinkNs, $spreadsheet);
                    break;
            }
        }
    }
    private function processTableRowGroup(DOMElement $childNode, string $tableNs, int &$rowID, string $worksheetName, string $officeNs, string $textNs, string $xlinkNs, Spreadsheet $spreadsheet) : void
    {
        foreach ($childNode->childNodes as $grandchildNode) {
            ...
            switch ($grandkey) {
                ...
                case 'table-header-rows':
                case 'table-rows':
                    $this->processTableHeaderRows($grandchildNode, $tableNs, $rowID, $worksheetName, $officeNs, $textNs, $xlinkNs, $spreadsheet);
                    break;
                case 'table-row-group':
                    $this->processTableRowGroup($grandchildNode, $tableNs, $rowID, $worksheetName, $officeNs, $textNs, $xlinkNs, $spreadsheet);
                    break;
            }
        }
    }
            ...
            if ($annotation->length > 0 && $annotation->item(0) !== null) {
                ...
                for ($textNodeIndex = 0; $textNodeIndex < $textNodeLength; ++$textNodeIndex) {
                    ...
                    if ($textNodeItem !== null) {
                        ...
                        if ($newLineOwed) {
                            $spreadsheet->getActiveSheet()->getComment($columnID . $rowID)->getText()->createText("\n");
                        }
                        $newLineOwed = \true;
                        $spreadsheet->getActiveSheet()->getComment($columnID . $rowID)->getText()->createText($this->parseRichText($text));
                    }
                }
            }
            // Content
            ...
            if (\count($paragraphs) > 0) {
                ...
                switch ($type) {
                    ...
                    case 'boolean':
                        ...
                        $dataValue = $cellData->getAttributeNS($officeNs, 'boolean-value') === 'true' ? \true : \false;
                        break;
                    case 'percentage':
    ...
    private function processTableHeaderColumns(DOMElement $childNode, string $tableNs, array $columnWidths, int &$tableColumnIndex, Spreadsheet $spreadsheet) : void
    {
        foreach ($childNode->childNodes as $grandchildNode) {
            ...
            switch ($grandkey) {
                case 'table-column':
                    $this->processTableColumn($grandchildNode, $tableNs, $columnWidths, $tableColumnIndex, $spreadsheet);
                    break;
            }
        }
    }
    ...
    private function processTableColumnGroup(DOMElement $childNode, string $tableNs, array $columnWidths, int &$tableColumnIndex, Spreadsheet $spreadsheet) : void
    {
        foreach ($childNode->childNodes as $grandchildNode) {
            ...
            switch ($grandkey) {
                ...
                case 'table-header-columns':
                case 'table-columns':
                    $this->processTableHeaderColumns($grandchildNode, $tableNs, $columnWidths, $tableColumnIndex, $spreadsheet);
                    break;
                case 'table-column-group':
                    $this->processTableColumnGroup($grandchildNode, $tableNs, $columnWidths, $tableColumnIndex, $spreadsheet);
                    break;
            }
        }
    }
