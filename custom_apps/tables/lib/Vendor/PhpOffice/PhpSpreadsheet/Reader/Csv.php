...
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Reader\Csv\Delimiter;
...
class Csv extends BaseReader
{
    ...
    public function listWorksheetInfo(string $filename) : array
    {
        ...
        $this->checkSeparator();
        $this->inferSeparator();
        $worksheetInfo = [['worksheetName' => 'Worksheet', 'lastColumnLetter' => 'A', 'lastColumnIndex' => 0, 'totalRows' => 0, 'totalColumns' => 0]];
        ...
        while (\is_array($rowData)) {
            ...
            foreach ($rowData as $rowDatum) {
                ...
                if (($rowDatum !== '' || $this->preserveNullString) && $this->readFilter->readCell($columnLetter, $currentRow)) {
                    ...
                    if ($numberFormatMask !== '') {
                        $sheet->getStyle($columnLetter . $outRow)->getNumberFormat()->setFormatCode($numberFormatMask);
                    }
                    // Set cell value
    ...
    private function convertFormattedNumber(mixed &$rowDatum) : string
    {
        ...
        if ($this->castFormattedNumberToNumeric === \true && \is_string($rowDatum)) {
            $numeric = \str_replace([$this->thousandsSeparator, $this->decimalSeparator], ['', '.'], $rowDatum);
            if (\is_numeric($numeric)) {
                $decimalPos = \strpos($rowDatum, $this->decimalSeparator);
                if ($this->preserveNumericFormatting === \true) {
                    $numberFormatMask = \str_contains($rowDatum, $this->thousandsSeparator) ? '#,##0' : '0';
                    if ($decimalPos !== \false) {
