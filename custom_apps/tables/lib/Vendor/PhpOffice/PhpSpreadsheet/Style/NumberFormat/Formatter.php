...
namespace OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Style\NumberFormat;

use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Calculation;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Reader\Xls\Color\BIFF8;
...
class Formatter extends BaseFormatter
{
    ...
    public static function toFormattedString($value, string $format, null|array|callable $callBack = null, bool $lessFloatPrecision = \false) : string
    {
        ...
        if (\preg_match(self::SECTION_SPLIT, $format) === 0 && \preg_match(self::SYMBOL_AT, $formatx) === 1) {
            ...
            $value = \str_replace(['$', '"'], ['\\$', self::QUOTE_REPLACEMENT], StringHelper::convertToString($value, lessFloatPrecision: $lessFloatPrecision));
        ...
        if ($format === NumberFormat::FORMAT_GENERAL || $format === NumberFormat::FORMAT_TEXT) {
            ...
            return self::adjustSeparators(StringHelper::convertToString($value, lessFloatPrecision: $lessFloatPrecision));
        }
