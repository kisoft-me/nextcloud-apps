...
namespace OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Style;

use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Chart\ChartColor;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Theme;
/** @internal */
class Font extends Supervisor
{
    ...
    public function applyFromArray(array $styleArray) : static
    {
        if ($this->isSupervisor) {
            ...
            if (isset($styleArray['color'])) {
                ...
                $this->getColor()->applyFromArray($temp);
            }
    ...
    public function getHashCode() : string
    {
        ...
        return \md5($this->name . $this->size . ($this->bold ? 't' : 'f') . ($this->italic ? 't' : 'f') . ($this->superscript ? 't' : 'f') . ($this->subscript ? 't' : 'f') . $this->underline . ($this->strikethrough ? 't' : 'f') . ($this->autoColor ? 't' : 'f') . $this->color->getHashCode() . $this->scheme . \implode('*', [$this->latin, $this->eastAsian, $this->complexScript, $this->strikeType, $this->hashChartColor($this->chartColor), $this->hashChartColor($this->underlineColor), (string) $this->baseLine, (string) $this->cap]) . __CLASS__);
    }
    /** @return mixed[] */
    ...
    {
        $this->cap = \in_array($cap, self::VALID_CAPS, \true) ? $cap : null;
        return $this;
    }
