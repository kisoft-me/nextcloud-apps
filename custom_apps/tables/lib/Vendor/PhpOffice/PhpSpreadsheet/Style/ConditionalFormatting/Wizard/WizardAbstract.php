...
abstract class WizardAbstract
{
    ...
    {
        if ($operandValueType === Wizard::VALUE_TYPE_LITERAL && \str_starts_with($operand, '"') && \str_ends_with($operand, '"')) {
    ...
    public static function reverseAdjustCellRef(string $condition, string $cellRange) : string
    {
        ...
        foreach ($splitCondition as &$value) {
            ...
            $i = $i === \false;
            if ($i) {
                $value = (string) \preg_replace_callback('/' . Calculation::CALCULATION_REGEXP_CELLREF_RELATIVE . '/i', fn($matches): string => self::reverseCellAdjustment($matches, $referenceColumnIndex, $referenceRow), $value);
            }
        }
        unset($value);
