...
class CellMatcher
{
    ...
    protected function processDuplicatesComparison(Conditional $conditional) : bool
    {
        ...
        $expression = \sprintf(self::COMPARISON_DUPLICATES_OPERATORS[$conditional->getConditionType()], $worksheetName, $this->conditionalRange, $this->cellConditionCheck($this->cell->getCalculatedValueString()));
        return $this->evaluateExpression($expression);
    }
    protected function processExpression(Conditional $conditional) : bool
    {
        ...
        $expression = (string) \preg_replace('/\\b' . $this->referenceCell . '\\b/i', (string) $temp, $expression);
        return $this->evaluateExpression($expression);
    }
