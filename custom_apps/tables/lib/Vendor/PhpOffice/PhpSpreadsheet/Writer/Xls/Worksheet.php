...
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Shared\Xls;
...
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Style\Protection;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\PageSetup;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\SheetView;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Writer\Exception as WriterException;
...
class Worksheet extends BIFFwriter
{
     ...
     * Sheet object.
     */
    public \OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $phpSheet;
    /**
     * Escher object corresponding to MSODRAWING.
     */
    private ?\OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Shared\Escher $escher = null;
    /**
     ...
     */
    public function __construct(int &$str_total, int &$str_unique, array &$str_table, array &$colors, Parser $parser, bool $preCalculateFormulas, \OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $phpSheet, ?Workbook $writerWorkbook = null)
    {
    ...
    public function close() : void
    {
        ...
        if (($defaultWidth = $phpSheet->getDefaultColumnDimension()->getWidth()) < 0) {
            $defaultWidth = \OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Shared\Font::getDefaultColumnWidthByFont($phpSheet->getParentOrThrow()->getDefaultStyle()->getFont());
        }
        ...
        for ($i = 0; $i <= $maxCol; ++$i) {
            ...
            if (isset($columnDimensions[$columnLetter])) {
                ...
                if ($columnDimension->getWidth() >= 0) {
                    $width = $columnDimension->getWidthForOutput(\true);
                }
            ...
            if ($cVal instanceof RichText) {
                ...
                foreach ($elements as $element) {
                    ...
                    if ($element instanceof Run) {
                        ...
                        $styleArray = $this->phpSheet->getParent()?->getCellXfCollection();
                        if ($styleArray !== null) {
     ...
     */
    public function setOutline(bool $visible = \true, bool $symbols_below = \true, bool $symbols_right = \true, bool $auto_style = \false) : void
    {
    ...
    {
        $bitmap_array = $bitmap instanceof GdImage ? $this->processBitmapGd($bitmap) : $this->processBitmap($bitmap);
     ...
     * Get Escher object.
     */
    public function getEscher() : ?\OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Shared\Escher
    {
        return $this->escher;
    }
    /**
     * Set Escher object.
     */
    public function setEscher(?\OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Shared\Escher $escher) : void
    {
        $this->escher = $escher;
    }
    /**
    ...
    private function writeDataValidity() : void
    {
        ...
        if (!empty($dataValidationCollection)) {
            ...
            foreach ($dataValidationCollection as $cellCoordinate => $dataValidation) {
                ...
                // explicit formula?
                if ($type == 0x3 && Preg::isMatch('/^\\".*\\"$/', $dataValidation->getFormula1())) {
                    $options |= 0x1 << 7;
                }
                // empty cells allowed
    ...
    private function writeCFRule(ConditionalHelper $conditionalFormulaHelper, Conditional $conditional, string $cellRange) : void
    {
        ...
        $flags |= 0 == $bBorderLeft ? 0x400 : 0;
        $flags |= 0 == $bBorderRight ? 0x800 : 0;
        $flags |= 0 == $bBorderTop ? 0x1000 : 0;
        $flags |= 0 == $bBorderBottom ? 0x2000 : 0;
        $flags |= 0 === $bBorderDiagTop ? 0x4000 : 0;
        // Top left to Bottom right border
        $flags |= 0 === $bBorderDiagBottom ? 0x8000 : 0;
        ...
        if ($bFormatFont == 1) {
            ...
            // Font Options
            $italicStrike = 0;
            if ($conditional->getStyle()->getFont()->getItalic() === \true) {
                $italicStrike |= 2;
            }
            if ($conditional->getStyle()->getFont()->getStrikethrough() === \true) {
                $italicStrike |= 0x80;
            }
            ...
            switch ($conditional->getStyle()->getFont()->getUnderline()) {
                case \OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Style\Font::UNDERLINE_NONE:
                    $dataBlockFont .= \pack('C', 0x0);
                    $fontUnderline = 0;
                    break;
                case \OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Style\Font::UNDERLINE_DOUBLE:
                    $dataBlockFont .= \pack('C', 0x2);
                    $fontUnderline = 0;
                    break;
                case \OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Style\Font::UNDERLINE_DOUBLEACCOUNTING:
                    $dataBlockFont .= \pack('C', 0x22);
                    $fontUnderline = 0;
                    break;
                case \OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Style\Font::UNDERLINE_SINGLE:
                    $dataBlockFont .= \pack('C', 0x1);
                    $fontUnderline = 0;
                    break;
                case \OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Style\Font::UNDERLINE_SINGLEACCOUNTING:
