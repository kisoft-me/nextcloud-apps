...
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Information\ErrorValue;
...
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Shared\XMLWriter;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Style\Conditional;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Style\ConditionalFormatting\ConditionalColorScale;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Style\ConditionalFormatting\ConditionalDataBar;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Style\ConditionalFormatting\ConditionalFormattingRuleExtension;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Style\ConditionalFormatting\ConditionalIconSet;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Style\Font;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\RowDimension;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\SheetView;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet as PhpspreadsheetWorksheet;
/** @internal */
class Worksheet extends WriterPart
{
    ...
    private function writeIconSetElements(XMLWriter $objWriter, ?ConditionalIconSet $iconSet) : void
    {
        ...
        foreach (['reverse' => $iconSet->getReverse(), 'showValue' => $iconSet->getShowValue(), 'custom' => $iconSet->getCustom()] as $attr => $value) {
        ...
        foreach ($iconSet->getCfvos() as $cfvo) {
            ...
            self::writeAttributeIf($objWriter, $cfvo->getValue() !== null, 'val', (string) $cfvo->getValue());
            self::writeAttributeIf($objWriter, $cfvo->getGreaterThanOrEqual() !== null, 'gte', $cfvo->getGreaterThanOrEqual() ? '1' : '0');
            $objWriter->endElement();
            // end cfvo
        }
        $objWriter->endElement();
    ...
    private function writeConditionalFormatting(XMLWriter $objWriter, PhpspreadsheetWorksheet $worksheet) : void
    {
        ...
        foreach ($worksheet->getConditionalStylesCollection() as $cellCoordinate => $conditionalStyles) {
            ...
            foreach ($conditionalStyles as $conditional) {
                ...
                self::writeAttributeIf($objWriter, $conditional->getConditionType() !== Conditional::CONDITION_COLORSCALE && $conditional->getConditionType() !== Conditional::CONDITION_DATABAR && $conditional->getConditionType() !== Conditional::CONDITION_ICONSET && $conditional->getNoFormatSet() === \false, 'dxfId', (string) $this->getParentWriter()->getStylesConditionalHashTable()->getIndexForHashCode($conditional->getHashCode()));
    ...
    private function writeCellInlineStr(XMLWriter $objWriter, string $mappedType, RichText|string $cellValue, ?Font $font) : void
    {
        ...
        if (!$cellValue instanceof RichText) {
            ...
            $textToWrite = StringHelper::controlCharacterPHP2OOXML($cellValue);
            ...
            $objWriter->startElement('is');
            $this->getParentWriter()->getWriterPartstringtable()->writeRichText($objWriter, $cellValue, null, $font);
            $objWriter->endElement();
        }
    }
    /**
    ...
    private function writeCellFormula(XMLWriter $objWriter, string $cellValue, Cell $cell) : void
    {
        ...
        if (($attributes['t'] ?? null) === 'array') {
            ...
            $objWriter->text(FunctionPrefix::addFunctionPrefixStripEquals($cellValue));
            $objWriter->endElement();
            if (\is_scalar($result) && $this->getParentWriter()->getOffice2003Compatibility() === \false && $this->getParentWriter()->getPreCalculateFormulas()) {
                $objWriter->writeElement('v', (string) $result);
            }
        } else {
            $objWriter->writeElement('f', FunctionPrefix::addFunctionPrefixStripEquals($cellValue));
            self::writeElementIf($objWriter, $this->getParentWriter()->getOffice2003Compatibility() === \false && $this->getParentWriter()->getPreCalculateFormulas() && $calculatedValue !== null, 'v', !\is_array($calculatedValue) && !\str_starts_with($calculatedValueString, '#') ? StringHelper::formatNumber($calculatedValueString) : '0');
        }
    }
    ...
    private function writeCell(XMLWriter $objWriter, PhpspreadsheetWorksheet $worksheet, string $cellAddress, array $flippedStringTable) : void
    {
        ...
        $styleArray = $this->getParentWriter()->getSpreadsheet()->getCellXfCollection();
