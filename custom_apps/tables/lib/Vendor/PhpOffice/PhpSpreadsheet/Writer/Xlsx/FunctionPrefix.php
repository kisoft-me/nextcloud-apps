...
class FunctionPrefix
{
    ...
    {
        $functionString = Preg::replaceCallback(Calculation::CALCULATION_REGEXP_CELLREF_SPILL, fn(array $matches) => 'ANCHORARRAY(' . \substr((string) $matches[0], 0, -1) . ')', $functionString);
    ...
    {
        $functionString = Preg::replace(['/\\b(CEILING|FLOOR)[.]ODS\\s*[(]/', '/\\b(CEILING|FLOOR)[.]XCL\\s*[(]/'], ['$1.MATH(', '$1('], $functionString);
