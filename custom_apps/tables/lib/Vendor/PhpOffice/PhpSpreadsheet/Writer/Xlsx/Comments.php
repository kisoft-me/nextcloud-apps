...
class Comments extends WriterPart
{
     ...
     */
    public function writeComments(\OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $worksheet) : string
    {
        // Create XML writer
        $objWriter = null;
     ...
     * @return string XML Output
     */
    public function writeVMLComments(\OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $worksheet) : string
    {
        // Create XML writer
        $objWriter = null;
        if ($this->getParentWriter()->getUseDiskCaching()) {
    ...
    private function writeVMLComment(XMLWriter $objWriter, string $cellReference, Comment $comment) : void
    {
        ...
        if (\in_array($alignment, self::VALID_HORIZONTAL_ALIGNMENT, \true)) {
