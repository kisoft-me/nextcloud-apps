...
namespace OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Writer\Ods\Cell;

use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Helper\Dimension;
...
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Style\Style as CellStyle;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\ColumnDimension;
...
class Style
{
    ...
    private function writeBorderStyle(string $direction, Border $border) : void
    {
        ...
        $this->writer->writeAttribute('fo:border-' . $direction, \sprintf('%s %s #%s', $this->mapBorderWidth($border), $this->mapBorderStyle($border), $border->getColor()->getRGB()));
    }
    ...
    protected function writeTextProperties(CellStyle $style) : void
    {
        ...
        if ($font->getAutoColor()) {
            $this->writer->writeAttribute('style:use-window-font-color', 'true');
        } else {
    ...
    public function writeDefaultRowStyle(RowDimension $rowDimension, int $sheetId) : void
    {
        $this->writer->startElement('style:style');
        $this->writer->writeAttribute('style:family', 'table-row');
        $this->writer->writeAttribute('style:name', \sprintf('%s%d', self::ROW_STYLE_PREFIX, $sheetId));
        $this->writeRowProperties($rowDimension);
        // End
        $this->writer->endElement();
        // Close style:style
