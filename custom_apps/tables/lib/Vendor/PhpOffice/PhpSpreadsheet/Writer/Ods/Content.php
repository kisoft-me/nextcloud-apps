...
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\RowCellIterator;
...
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Writer\Ods\Cell\Comment;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Writer\Ods\Cell\Style;
/**
...
class Content extends WriterPart
{
    ...
    private function writeRows(XMLWriter $objWriter, Worksheet $sheet, int $sheetIndex) : void
    {
        ...
        foreach ($rows as $row) {
            $cellIterator = $row->getCellIterator(iterateOnlyExistingCells: \true);
            $cellIterator->rewind();
            ...
            if ($cellIterator->valid() || $rowStyleExists) {
                if ($spanRow) {
                    ...
                    $objWriter->writeAttribute('table:number-rows-repeated', (string) $spanRow);
                    $objWriter->endElement();
                    $spanRow = 0;
                }
                $objWriter->startElement('table:table-row');
                if ($rowStyleExists) {
                    ...
                    $objWriter->writeAttribute('table:style-name', \sprintf('%s%d', Style::ROW_STYLE_PREFIX, $sheetIndex));
                }
