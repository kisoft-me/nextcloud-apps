...
namespace OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Cell;
...
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Shared\Date as SharedDate;
...
class Cell implements Stringable
{
    ...
    public function getFormattedValue() : string
    {
        ...
        $formattedValue = (string) NumberFormat::toFormattedString($this->getCalculatedValueString(), (string) $this->getStyle()->getNumberFormat()->getFormatCode(\true));
    ...
    public function setValueExplicit(mixed $value, string $dataType = DataType::TYPE_STRING) : self
    {
        ...
        switch ($dataType) {
            ...
            case DataType::TYPE_INLINE:
                // Rich text
                $value2 = StringHelper::convertToString($value, \true);
                // Cells?->Worksheet?->Spreadsheet
            ...
            case DataType::TYPE_FORMULA:
                $this->value = StringHelper::convertToString($value, \true);
                break;
        ...
        if (isset($spreadsheet) && $spreadsheet->getIndex($worksheet, \true) >= 0) {
     ...
     * @throws CalculationException
     */
    public function getCalculatedValue(bool $resetLog = \true) : mixed
    {
        $title = 'unknown';
        ...
        if ($this->dataType === DataType::TYPE_FORMULA) {
            try {
                ...
                if (!\is_array($result) && $calculation->getInstanceArrayReturnType() === Calculation::RETURN_ARRAY_AS_ARRAY && $oldAttributesT === 'array' && ($oldAttributesRef === $coordinate || $oldAttributesRef === "{$coordinate}:{$coordinate}")) {
                    $result = [$result];
                }
                ...
                throw new CalculationException($title . '!' . $this->getCoordinate() . ' -> ' . $ex->getMessage(), $ex->getCode(), $ex);
            }
            SharedDate::setExcelCalendar($currentCalendar);
     ...
     */
    public function setCalculatedValue(mixed $originalValue, bool $tryNumeric = \true) : self
    {
