...
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Document\Security;
...
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Worksheet\Iterator;
...
class Spreadsheet implements JsonSerializable
{
     ...
     */
    private bool $showHorizontalScroll = \true;
    /**
     * Controls visibility of the horizonal scroll bar in the application.
     */
    private bool $showVerticalScroll = \true;
    /**
     * Controls visibility of the sheet tabs in the application.
     */
    private bool $showSheetTabs = \true;
    /**
     ...
     * interface.
     */
    private bool $autoFilterDateGrouping = \true;
    /**
    ...
    {
        if (!isset($this->calculationEngine)) {
            //* @phpstan-ignore-line
            return null;
        }
        return $this->calculationEngine;
    ...
    public function duplicateWorksheetByTitle(string $title) : Worksheet
    {
        ...
        return $this->addSheet($clone, $index, \true);
    }
    /**
     * Add sheet.
     *
    ...
    {
        $trimWorksheetName = StringHelper::strToUpper(\trim($worksheetName, "'"));
        foreach ($this->workSheetCollection as $worksheet) {
    ...
    {
        return \array_filter($this->definedNames, fn(DefinedName $definedName): bool => $definedName->isFormula() === self::DEFINED_NAME_IS_RANGE);
    }
    /**
    ...
    {
        return \array_filter($this->definedNames, fn(DefinedName $definedName): bool => $definedName->isFormula() === self::DEFINED_NAME_IS_FORMULA);
    }
    /**
    ...
    public function getDefinedName(string $definedName, ?Worksheet $worksheet = null) : ?DefinedName
    {
        ...
        if ($definedName !== '') {
            ...
            foreach ($this->definedNames as $dn) {
                ...
                if (!$dn->getLocalOnly() && $definedName === $upper) {
                    $returnValue = $dn;
                    break;
                }
            }
            ...
            if ($worksheet !== null) {
                ...
                foreach ($this->definedNames as $dn) {
                    ...
                    if ($dn->getLocalOnly() && $upper === $definedName && $upperTitle === $wsTitle) {
                        return $dn;
                    }
                }
            }
    ...
    public function __clone()
    {
        ...
        $this->cellXfSupervisor->bindParent($this);
        $usedKeys['cellXfSupervisor'] = \true;
        $oldCalc = $this->calculationEngine;
        $this->calculationEngine = new Calculation($this);
        $this->calculationEngine->setSuppressFormulaErrors($oldCalc->getSuppressFormulaErrors())->setCalculationCacheEnabled($oldCalc->getCalculationCacheEnabled())->setBranchPruningEnabled($oldCalc->getBranchPruningEnabled())->setInstanceArrayReturnType($oldCalc->getInstanceArrayReturnType());
        $usedKeys['calculationEngine'] = \true;
        ...
        $usedKeys['cellStyleXfCollection'] = \true;
        ...
        $usedKeys['workSheetCollection'] = \true;
