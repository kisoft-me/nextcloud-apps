...
class Drawing
{
     ...
     */
    public static function pixelsToCellDimension(int $pixelValue, \OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Style\Font $defaultFont) : int|float
    {
        // Font name and size
        ...
        if (isset(Font::DEFAULT_COLUMN_WIDTHS[$name][$sizex])) {
            ...
            return $pixelValue * Font::DEFAULT_COLUMN_WIDTHS[$name][$sizex]['width'] / Font::DEFAULT_COLUMN_WIDTHS[$name][$sizex]['px'];
        }
        // We don't have data for this particular font and size, use approximation by
        // extrapolating from Calibri 11
        return $pixelValue * 11 * Font::DEFAULT_COLUMN_WIDTHS['Calibri'][11]['width'] / Font::DEFAULT_COLUMN_WIDTHS['Calibri'][11]['px'] / $size;
    }
    /**
     ...
     * @return int Value in pixels
     */
    public static function cellDimensionToPixels(float $cellWidth, \OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Style\Font $defaultFont) : int
    {
        // Font name and size
        $name = $defaultFont->getName();
        $size = $defaultFont->getSize();
        ...
        if (isset(Font::DEFAULT_COLUMN_WIDTHS[$name][$sizex])) {
            // Exact width can be determined
            $colWidth = $cellWidth * Font::DEFAULT_COLUMN_WIDTHS[$name][$sizex]['px'] / Font::DEFAULT_COLUMN_WIDTHS[$name][$sizex]['width'];
        } else {
            // We don't have data for this particular font and size, use approximation by
            // extrapolating from Calibri 11
            $colWidth = $cellWidth * $size * Font::DEFAULT_COLUMN_WIDTHS['Calibri'][11]['px'] / Font::DEFAULT_COLUMN_WIDTHS['Calibri'][11]['width'] / 11;
        }
