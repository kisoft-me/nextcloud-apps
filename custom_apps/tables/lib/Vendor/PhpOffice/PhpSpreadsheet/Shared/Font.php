...
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Style\Font as FontStyle;
...
class Font
{
     ...
     */
    public static function getTrueTypeFontFileFromFont(FontStyle $font, bool $checkPath = \true) : string
    {
    ...
    public static function getDefaultColumnWidthByFont(FontStyle $font, bool $returnAsPixels = \false) : float|int
    {
        ...
        if (isset(self::DEFAULT_COLUMN_WIDTHS[$font->getName()][$sizex])) {
            ...
            $columnWidth = $returnAsPixels ? self::DEFAULT_COLUMN_WIDTHS[$font->getName()][$sizex]['px'] : self::DEFAULT_COLUMN_WIDTHS[$font->getName()][$sizex]['width'];
        } else {
    ...
    private static function findFontFile(string $startDirectory, string $desiredFont) : ?string
    {
        ...
        if (\file_exists("{$startDirectory}/{$desiredFont}")) {
            ...
            $iterations = 0;
            $it = new RecursiveDirectoryIterator($startDirectory, RecursiveDirectoryIterator::SKIP_DOTS | RecursiveDirectoryIterator::FOLLOW_SYMLINKS);
            foreach (new RecursiveIteratorIterator($it, RecursiveIteratorIterator::LEAVES_ONLY, RecursiveIteratorIterator::CATCH_GET_CHILD) as $filex) {
                /** @var string */
                $file = $filex;
