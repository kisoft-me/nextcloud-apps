...
namespace OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Shared\Trend;
...
class PowerBestFit extends BestFit
{
    ...
    {
        $adjustedYValues = \array_map(fn($value): float => $value < 0.0 ? 0 - \log(\abs($value)) : \log($value), $yValues);
        $adjustedXValues = \array_map(fn($value): float => $value < 0.0 ? 0 - \log(\abs($value)) : \log($value), $xValues);
     ...
     * @param float[] $xValues The set of X-values for this regression
     */
    public function __construct(array $yValues, array $xValues = [], bool $const = \true)
    {
