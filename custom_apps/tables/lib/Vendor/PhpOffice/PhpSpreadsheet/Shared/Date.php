...
namespace OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Shared;

use DateTime;
use DateTimeInterface;
use DateTimeZone;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Calculation\DateTimeExcel;
...
class Date
{
    ...
    public static function excelToDateTimeObject(float|int $excelTimestamp, null|DateTimeZone|string $timeZone = null) : DateTime
    {
        ...
        $interval = $days . ' days';
        return $baseDate->modify($interval)->setTime($hours, $minutes, $seconds, $microseconds);
    }
    /**
    ...
    public static function dateTimeToExcel(DateTimeInterface $dateValue) : float
    {
        ...
        return self::formattedPHPToExcel((int) $dateValue->format('Y'), (int) $dateValue->format('m'), (int) $dateValue->format('d'), (int) $dateValue->format('H'), (int) $dateValue->format('i'), $seconds);
    }
    /**
     ...
     */
    public static function isDateTime(Cell $cell, mixed $value = null, bool $dateWithoutTimeOkay = \true) : bool
    {
        $result = \false;
        ...
        if ($worksheet !== null && $spreadsheet !== null) {
            ...
            try {
                if ($value === null) {
                    $value = Functions::flattenSingleValue($cell->getCalculatedValue());
                }
                if (\is_numeric($value)) {
                    $result = self::isDateTimeFormat($worksheet->getStyle($cell->getCoordinate())->getNumberFormat(), $dateWithoutTimeOkay);
                    /** @var float|int $value */
     ...
     */
    public static function isDateTimeFormat(NumberFormat $excelFormatCode, bool $dateWithoutTimeOkay = \true) : bool
    {
     ...
     */
    public static function isDateTimeFormatCode(string $excelFormatCode, bool $dateWithoutTimeOkay = \true) : bool
    {
