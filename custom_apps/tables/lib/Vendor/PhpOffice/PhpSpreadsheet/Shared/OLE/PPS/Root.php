...
namespace OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Shared\OLE\PPS;
...
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Shared\OLE\PPS;
/**
        ...
        for ($i = 0; $i < $iCount; ++$i) {
            if ($raList[$i]->Type != OLE::OLE_PPS_TYPE_DIR) {
                ...
                if ($raList[$i]->Size >= OLE::OLE_DATA_SIZE_SMALL || $raList[$i]->Type == OLE::OLE_PPS_TYPE_ROOT && isset($raList[$i]->_data)) {
                    ...
                    $iStBlk += (int) \floor($raList[$i]->Size / $this->bigBlockSize) + ($raList[$i]->Size % $this->bigBlockSize ? 1 : 0);
                }
            }
        }
    }
    ...
    private function makeSmallData(array &$raList) : string
    {
        ...
        for ($i = 0; $i < $iCount; ++$i) {
            ...
            if ($raList[$i]->Type == OLE::OLE_PPS_TYPE_FILE) {
                ...
                if ($raList[$i]->Size < OLE::OLE_DATA_SIZE_SMALL) {
                    $iSmbCnt = (int) \floor($raList[$i]->Size / $this->smallBlockSize) + ($raList[$i]->Size % $this->smallBlockSize ? 1 : 0);
                    // Add to SBD
                    $jB = $iSmbCnt - 1;
                    for ($j = 0; $j < $jB; ++$j) {
