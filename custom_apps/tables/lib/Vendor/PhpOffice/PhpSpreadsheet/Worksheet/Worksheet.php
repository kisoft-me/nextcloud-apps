...
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Cell\AddressRange;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Cell\Cell;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Cell\CellAddress;
...
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Cell\IValueBinder;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Chart\Chart;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Collection\Cells;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Collection\CellsFactory;
...
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Shared;
...
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Style\Alignment;
...
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Style\Protection as StyleProtection;
...
class Worksheet
{
     ...
     * Show gridlines?
     */
    private bool $showGridlines = \true;
    /**
     * Print gridlines?
     */
     ...
     */
    private bool $showRowColHeaders = \true;
    /**
     * Show summary below? (Row/Column outline).
     */
    private bool $showSummaryBelow = \true;
    /**
     * Show summary right? (Row/Column outline).
     */
    private bool $showSummaryRight = \true;
    /**
    ...
    {
        Calculation::getInstanceOrNull($this->parent)?->clearCalculationCacheForWorksheet($this->title);
    ...
    private static function checkSheetCodeName(string $sheetCodeName) : string
    {
        ...
        if (\str_replace(self::INVALID_CHARACTERS, '', $sheetCodeName) !== $sheetCodeName || StringHelper::substring($sheetCodeName, -1, 1) == '\'' || StringHelper::substring($sheetCodeName, 0, 1) == '\'') {
     ...
     * @return string[]
     */
    public function getCoordinates(bool $sorted = \true) : array
    {
        if (!isset($this->cellCollection)) {
            //* @phpstan-ignore-line
            return [];
            ...
            foreach ($this->getCoordinates(\false) as $coordinate) {
                ...
                if ($cell !== null && isset($autoSizes[$this->cellCollection->getCurrentColumn()])) {
                    ...
                    if (!$isMerged || $isMergedButProceed) {
                        ...
                        // Calculated value
                        // To formatted string
                        $cellValue = NumberFormat::toFormattedString($cell->getCalculatedValueString(), (string) $this->getParentOrThrow()->getCellXfByIndex($cell->getXfIndex())->getNumberFormat()->getFormatCode(\true));
                        if ($cellValue !== '') {
     ...
     * @return $this
     */
    public function setTitle(string $title, bool $updateFormulaCellReferences = \true, bool $validate = \true) : static
    {
    ...
    private function getWorksheetAndCoordinate(string $coordinate) : array
    {
        ...
        if (\str_contains($coordinate, '!')) {
            $worksheetReference = self::extractSheetTitle($coordinate, \true, \true);
    ...
    {
        return $this->parent?->getCellXfByIndexOrNull(($this->rowDimensions[$row] ?? null)?->getXfIndex());
    }
    ...
    {
        return $this->parent?->getCellXfByIndexOrNull(($this->columnDimensions[$column] ?? null)?->getXfIndex());
    }
    /**
     * Get style for cell.
     *
     ...
     * @return Conditional[]
     */
    public function getConditionalStyles(string $coordinate, bool $firstOnly = \true) : array
    {
    ...
    {
        $this->xSplit = $xSplit;
        if (\in_array($this->paneState, self::VALIDFROZENSTATE, \true)) {
    ...
    {
        $this->paneState = \in_array($paneState, self::VALIDPANESTATE, \true) ? $paneState : '';
        if (\in_array($this->paneState, self::VALIDFROZENSTATE, \true)) {
            $this->freezePane([$this->xSplit + 1, $this->ySplit + 1], $this->topLeftCell, $this->paneState === self::PANE_FROZENSPLIT);
        } else {
            $this->freezePane = null;
     ...
     *               or as an array of [$columnIndex, $row] (e.g. [3, 5]), or a CellAddress object.
     */
    public function getComment(CellAddress|string|array $cellCoordinate, bool $attachNew = \true) : Comment
    {
        $cellAddress = Functions::trimSheetFromCellReference(Validations::validateCellAddress($cellCoordinate));
        if (Coordinate::coordinateIsRange($cellAddress)) {
            throw new Exception('Cell coordinate string can not be a range of cells.');
    ...
    protected function cellToArray(Cell $cell, bool $calculateFormulas, bool $formatData, mixed $nullValue, bool $lessFloatPrecision = \false) : mixed
    {
        ...
        if ($cell->getValue() !== null) {
            ...
            if ($formatData) {
                ...
                $returnValue = NumberFormat::toFormattedString($returnValuex, $style->getNumberFormat()->getFormatCode() ?? NumberFormat::FORMAT_GENERAL, lessFloatPrecision: $lessFloatPrecision);
            }
        }
        return $returnValue;
    }
     ...
     * @return mixed[][]
     */
    public function rangeToArray(string $range, mixed $nullValue = null, bool $calculateFormulas = \true, bool $formatData = \true, bool $returnCellRef = \false, bool $ignoreHidden = \false, bool $reduceArrays = \false, bool $lessFloatPrecision = \false) : array
    {
        $returnValue = [];
        // Loop through rows
     ...
     * @return mixed[][]
     */
    public function rangesToArray(string $ranges, mixed $nullValue = null, bool $calculateFormulas = \true, bool $formatData = \true, bool $returnCellRef = \false, bool $ignoreHidden = \false, bool $reduceArrays = \false, bool $lessFloatPrecision = \false) : array
    {
        $returnValue = [];
     ...
     */
    public function rangeToArrayYieldRows(string $range, mixed $nullValue = null, bool $calculateFormulas = \true, bool $formatData = \true, bool $returnCellRef = \false, bool $ignoreHidden = \false, bool $reduceArrays = \false, bool $lessFloatPrecision = \false)
    {
        ...
        for ($row = $minRow; $row <= $maxRow; ++$row) {
            if ($ignoreHidden === \true && $this->isRowVisible($row) === \false) {
                continue;
            }
    ...
    private function buildNullRow(mixed $nullValue, string $minCol, string $maxCol, bool $returnCellRef, bool $ignoreHidden, array &$hiddenColumns) : array
    {
        ...
        for ($col = $minCol; $col !== $maxCol; StringHelper::stringIncrement($col)) {
            if ($ignoreHidden === \true && $this->columnDimensionExists($col) && $this->getColumnDimension($col)->getVisible() === \false) {
                $hiddenColumns[$col] = \true;
            } else {
     ...
     * @return mixed[][]
     */
    public function namedRangeToArray(string $definedName, mixed $nullValue = null, bool $calculateFormulas = \true, bool $formatData = \true, bool $returnCellRef = \false, bool $ignoreHidden = \false, bool $reduceArrays = \false, bool $lessFloatPrecision = \false) : array
    {
        $retVal = [];
     ...
     * @return mixed[][]
     */
    public function toArray(mixed $nullValue = null, bool $calculateFormulas = \true, bool $formatData = \true, bool $returnCellRef = \false, bool $ignoreHidden = \false, bool $reduceArrays = \false, bool $lessFloatPrecision = \false) : array
    {
        // Garbage collect...
        $this->garbageCollect();
     ...
     * @return $this
     */
    public function setCodeName(string $codeName, bool $validate = \true) : static
    {
        // Is this a 'rename' or not?
    ...
    {
        if ($this->getProtection()->getsheet() !== \true) {
            return \false;
        }
     ...
     */
    public function copyCells(string $fromCell, string $toCells, bool $copyStyle = \true) : void
    {
    ...
    public function calculateArrays(bool $preCalculateFormulas = \true) : void
    {
    ...
    public function copyFormula(string $fromCell, string $toCell) : void
    {
        ...
        if (\is_string($formula) && $this->getCell($fromCell)->getDataType() === DataType::TYPE_FORMULA) {
            ...
            $newFormula = $helper->updateFormulaReferences($formula, 'A1', $toColInt - $fromColInt, $toRow - $fromRow);
        }
