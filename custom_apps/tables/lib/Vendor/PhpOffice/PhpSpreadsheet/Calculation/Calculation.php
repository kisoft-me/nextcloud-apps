...
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Engine\BranchPruner;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Engine\CyclicReferenceStack;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Engine\Logger;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Engine\Operands;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Information\ExcelError;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Token\Stack;
...
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\NamedRange;
...
class Calculation extends CalculationLocale
{
     ...
     */
    private bool $calculationCacheEnabled = \true;
    private BranchPruner $branchPruner;
    private bool $branchPruningEnabled = \true;
    /**
     ...
     */
    private const EXCEL_CONSTANTS = ['TRUE' => \true, 'FALSE' => \false, 'NULL' => null];
     ...
     */
    public function calculateCellValue(?Cell $cell = null, bool $resetLog = \true) : mixed
    {
        if ($cell === null) {
            return null;
        }
        ...
        try {
            ...
            if (\is_string($value) && $cell->getDataType() === DataType::TYPE_FORMULA) {
                $value = \preg_replace_callback(self::CALCULATION_REGEXP_CELLREF_SPILL, fn(array $matches) => 'ANCHORARRAY(' . \substr($matches[0], 0, -1) . ')', $value);
            }
    ...
    {
        $formula = \preg_replace_callback(self::CALCULATION_REGEXP_CELLREF_SPILL, fn(array $matches) => 'ANCHORARRAY(' . \substr($matches[0], 0, -1) . ')', $formula) ?? $formula;
     ...
     */
    private const COMPARISON_OPERATORS = ['>' => \true, '<' => \true, '=' => \true, '>=' => \true, '<=' => \true, '<>' => \true];
    /**
     *    Operator Precedence.
        ...
        while (\true) {
            ...
            if ($opCharacter === '-' && !$expectingOperator) {
                ...
                if (\preg_match('/^' . self::CALCULATION_REGEXP_FUNCTION . '$/miu', $val, $matches)) {
                    ...
                    if (isset($phpSpreadsheetFunctions[\strtoupper($matches[1])]) || isset(self::$controlFunctions[\strtoupper($matches[1])])) {
                        // it's a function
                    ...
                    } elseif (isset(self::EXCEL_CONSTANTS[\trim(\strtoupper($val))])) {
    ...
    private function processTokenStack(false|array $tokens, ?string $cellID = null, ?Cell $cell = null)
    {
        ...
        foreach ($tokens as $tokenIdx => $tokenData) {
            ...
            if ($tokenData['type'] === 'Cell Reference' && isset($tokens[$tokenIdx + 1]) && $tokens[$tokenIdx + 1]['type'] === 'Operand Count for Function ANCHORARRAY()') {
                //* @phpstan-ignore-line
                $this->processingAnchorArray = \true;
            }
            ...
            if ($token instanceof Operands\StructuredReference) {
                ...
                switch ($token) {
                    ...
                    case ':':
                        ...
                        if (\str_contains($operand1Data['reference'] ?? '', '!')) {
                            [$sheet1, $operand1Data['reference']] = Worksheet::extractSheetTitle($operand1Data['reference'], \true, \true);
                        } else {
                        ...
                        [$sheet2, $operand2Data['reference']] = Worksheet::extractSheetTitle($op2ref, \true, \true);
                        if (empty($sheet2)) {
                            $sheet2 = $sheet1;
                        }
                        if ($sheet1 === $sheet2) {
                    ...
                    case '&':
                        ...
                        if (\is_array($operand1) || \is_array($operand2)) {
                            ...
                            for ($row = 0; $row < $rows; ++$row) {
                                for ($column = 0; $column < $columns; ++$column) {
                                    ...
                                    if (Information\ErrorValue::isError($op1x)) {
                                        ...
                                        /** @var string $op1x */
                                        /** @var string $op2x */
                                        $operand1[$row][$column] = StringHelper::substring($op1x . $op2x, 0, DataType::MAX_STRING_LENGTH);
                                    }
                                }
                            }
                            $result = $operand1;
                            ...
                            if (Information\ErrorValue::isError($operand1)) {
                                ...
                                //* @phpstan-ignore-line
                                $result = StringHelper::substring($result, 0, DataType::MAX_STRING_LENGTH);
                ...
                if (isset($phpSpreadsheetFunctions[$functionName]) || isset(self::$controlFunctions[$functionName])) {
                    ...
                    for ($i = 0; $i < $argCount; ++$i) {
                        $arg = $stack->pop();
                        $a = $argCount - $i - 1;
                        if ($passByReference && isset($phpSpreadsheetFunctions[$functionName]['passByReference'][$a]) && $phpSpreadsheetFunctions[$functionName]['passByReference'][$a]) {
                            /** @var mixed[] $arg */
                            ...
                            /** @var mixed[] $arg */
                            if ($arg['type'] === 'Empty Argument' && \in_array($functionName, ['MIN', 'MINA', 'MAX', 'MAXA', 'IF'], \true)) {
                ...
                if (isset(self::EXCEL_CONSTANTS[\strtoupper($token ?? '')])) {
                    ...
                    $specifiedWorksheet = \trim($matches[2], "'");
                    ...
                    if ($namedRange === null && $this->spreadsheet !== null) {
                        ...
                        if ($table !== null) {
                            ...
                            $tableRangeString = '$' . $tableRange[0][0] . '$' . $tableRange[0][1] . ':' . '$' . $tableRange[1][0] . '$' . $tableRange[1][1];
    ...
    private function executeNumericBinaryOperation(mixed $operand1, mixed $operand2, string $operation, Stack &$stack) : mixed
    {
        ...
        if (Functions::getCompatibilityMode() != Functions::COMPATIBILITY_OPENOFFICE && (\is_string($operand1) && !\is_numeric($operand1) && $operand1 !== '' || \is_string($operand2) && !\is_numeric($operand2) && $operand2 !== '')) {
     ...
     */
    public function extractCellRange(string &$range = 'A1', ?Worksheet $worksheet = null, bool $resetLog = \true, bool $createCell = \false) : array
    {
        // Return value
        /** @var mixed[][] */
        $returnValue = [];
        if ($worksheet !== null) {
            ...
            if (\str_contains($range, '!')) {
                [$worksheetName, $range] = Worksheet::extractSheetTitle($range, \true, \true);
     ...
     */
    public function extractNamedRange(string &$range = 'A1', ?Worksheet $worksheet = null, bool $resetLog = \true) : string|array
    {
        // Return value
        $returnValue = [];
        if ($worksheet !== null) {
    ...
    private function evaluateDefinedName(Cell $cell, DefinedName $namedRange, Worksheet $cellWorksheet, Stack $stack, bool $ignoreScope = \false) : mixed
    {
        ...
        $originalCoordinate = $cell->getCoordinate();
        $recursiveCalculationCell = $definedNameType !== 'Formula' && $definedNameWorksheet !== null && $definedNameWorksheet !== $cellWorksheet ? $definedNameWorksheet->getCell('A1') : $cell;
