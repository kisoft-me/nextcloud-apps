...
class Random
{
    ...
    public static function randArray(mixed $rows = 1, mixed $columns = 1, mixed $min = 0, mixed $max = 1, bool $wholeNumber = \false) : string|array
    {
        ...
        return \array_chunk(\array_map(fn(): int|float => $wholeNumber ? \mt_rand((int) $min, (int) $max) : \mt_rand() / \mt_getrandmax() * ($max - $min) + $min, \array_fill(0, $rows * $columns, $min)), \max($columns, 1));
    }
}
