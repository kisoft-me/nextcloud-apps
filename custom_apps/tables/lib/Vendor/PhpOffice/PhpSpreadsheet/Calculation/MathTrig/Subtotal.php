...
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Statistical;
...
class Subtotal
{
    ...
    {
        return \array_filter($args, function ($index) use($cellReference) : bool {
            $explodeArray = \explode('.', $index);
            $row = $explodeArray[1] ?? '';
            ...
            if ($cellReference->getWorksheet()->cellExists($column . $row)) {
                ...
                $cellFormula = !\preg_match('/^=.*\\b(SUBTOTAL|AGGREGATE)\\s*\\(/i', $cellReference->getWorksheet()->getCell($column . $row)->getValueString());
