...
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Database\DAverage;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Database\DCount;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Database\DMax;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Database\DMin;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Database\DSum;
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Exception as CalcException;
...
class Conditional
{
    ...
    public static function AVERAGEIF(mixed $range, null|array|string $condition, mixed $averageRange = []) : null|int|float|string
    {
        if (!\is_array($range) || !\is_array($averageRange) || \array_key_exists(0, $range) || \array_key_exists(0, $averageRange)) {
            ...
            if (\in_array($refError, [$range, $averageRange], \true)) {
                return $refError;
            }
    ...
    public static function COUNTIF(mixed $range, null|array|string $condition) : string|int
    {
        ...
        $range = \array_filter(Functions::flattenArray($range), fn($value): bool => $value !== null && $value !== '');
    ...
    {
        if (!\is_array($range) || \array_key_exists(0, $range) || !\is_array($sumRange) || \array_key_exists(0, $sumRange)) {
            $refError = ExcelError::REF();
            if (\in_array($refError, [$range, $sumRange], \true)) {
                return $refError;
            }
            throw new CalcException('Must specify range of cells, not any kind of literal');
        }
