...
namespace OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Internal;
...
class ExcelArrayPseudoFunctions
{
    public static function single(string $cellReference, Cell $cell) : mixed
    {
        ...
        [$referenceWorksheetName, $referenceCellCoordinate] = Worksheet::extractSheetTitle($cellReference, \true, \true);
        ...
        $referenceCell = $referenceWorksheetName === '' ? $worksheet->getCell((string) $referenceCellCoordinate) : $worksheet->getParentOrThrow()->getSheetByNameOrThrow((string) $referenceWorksheetName)->getCell((string) $referenceCellCoordinate);
