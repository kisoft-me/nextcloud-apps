...
class Thai
{
    ...
    public static function getBahtText(mixed $number) : array|string
    {
        ...
        $text = $isNegative ? self::THAI_MINUS : '';
        if ($hasWhole) {
            $text .= self::convertLarge($baht) . self::THAI_BAHT;
        }
        $text .= $hasFraction ? self::convertBlock($satang) . self::THAI_SATANG : self::THAI_INTEGER;
        return $text;
    }
    private static function convertLarge(string $digits) : string
    {
        ...
        return \implode(self::THAI_UNITS[6], \array_map(self::convertBlock(...), $chunks));
    }
