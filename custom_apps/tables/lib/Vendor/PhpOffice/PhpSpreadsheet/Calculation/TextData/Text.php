...
namespace OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Calculation\TextData;
...
use OCA\Tables\Vendor\PhpOffice\PhpSpreadsheet\Calculation\Exception as CalcExp;
...
class Text
{
    ...
    public static function length(mixed $value = '') : array|int|string
    {
        ...
        try {
            $value = Helpers::extractString($value, \true);
        } catch (CalcExp $e) {
            return $e->getMessage();
        }
    ...
    public static function exact(mixed $value1, mixed $value2) : array|bool|string
    {
        ...
        try {
            $value1 = Helpers::extractString($value1, \true);
            $value2 = Helpers::extractString($value2, \true);
        } catch (CalcExp $e) {
            return $e->getMessage();
        }
     ...
     */
    public static function split(mixed $text, $columnDelimiter = null, $rowDelimiter = null, bool $ignoreEmpty = \false, bool $matchMode = \true, mixed $padding = '#N/A') : array|string
    {
        $text = Functions::flattenSingleValue($text);
        if (ErrorValue::isError($text, \true)) {
            return StringHelper::convertToString($text);
        }
        $flags = self::matchFlags($matchMode);
        if ($rowDelimiter !== null) {
            $delimiter = self::buildDelimiter($rowDelimiter);
            $rows = $delimiter === '()' ? [$text] : Preg::split("/{$delimiter}/{$flags}", StringHelper::convertToString($text));
        } else {
            $rows = [$text];
        }
        if ($ignoreEmpty === \true) {
        ...
        if ($columnDelimiter !== null) {
            ...
            \array_walk($rows, function (&$row) use($delimiter, $flags, $ignoreEmpty) : void {
                $row = $delimiter === '()' ? [$row] : Preg::split("/{$delimiter}/{$flags}", StringHelper::convertToString($row));
                if ($ignoreEmpty === \true) {
    ...
    private static function applyPadding(array $rows, mixed $padding) : array
    {
        ...
        return \array_map(fn(array $row): array => \count($row) < $columnCount ? \array_merge($row, \array_fill(0, $columnCount - \count($row), $padding)) : $row, $rows);
    }
    /**
    ...
    private static function buildDelimiter($delimiter) : string
    {
        ...
        if (\is_array($delimiter) && \count($valueSet) > 1) {
            ...
            $quotedDelimiters = \array_map(fn(?string $delimiter): string => \preg_quote($delimiter ?? '', '/'), $valueSet);
