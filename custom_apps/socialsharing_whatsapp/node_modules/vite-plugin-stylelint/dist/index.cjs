const require_utils = require("./utils-B_gQwkEH.cjs");
...
vite = require_utils.__toESM(vite);
//#region src/index.ts
...
function StylelintPlugin(userOptions = {}) {
	...
	const plugin = {
		...
		async transform(_, id) {
			...
			const filteredIds = ids.filter((id) => !require_utils.shouldIgnoreModule(id, filter));
			debug$1(`filteredIds: ${filteredIds}`);
			const shouldIgnore = filteredIds.length === 0;
			debug$1(`should ignore: ${shouldIgnore}`);
			if (shouldIgnore) return;
			const filePaths = filteredIds.map((id) => require_utils.getFilePath(id));
